...
extension Data
{
  ...
  /// stream according to [RFC-1951](https://tools.ietf.org/html/rfc1951).
  /// - returns: uncompressed data
  public func blockInflate() -> Data?
  {
    return self.withUnsafeBytes { (sourcePtr: UnsafePointer<UInt8>) -> Data? in
      return blockPerform(COMPRESSION_ZLIB, source: sourcePtr, sourceSize: count)
    }
  }
  ...
  /// - parameter skipCheckSumValidation: skip the adler32 checksum validation. default: true
  /// - returns: uncompressed data
  public func blockUnzip(skipCheckSumValidation: Bool = true) -> Data?
  {
    // 2 byte header + 4 byte adler32 checksum
    let overhead = 6
    guard count > overhead else { return nil }
    ...
    let cresult: Data? = withUnsafeBytes { (ptr: UnsafePointer<UInt8>) -> Data? in
      let source = ptr.advanced(by: 2)
      return blockPerform(COMPRESSION_ZLIB, source: source, sourceSize: count - overhead)
    }
...
fileprivate func blockPerform(_ algorithm: compression_algorithm, source: UnsafePointer<UInt8>, sourceSize: Int) -> Data?
{
  var result: Data? = nil
  
  let destBufferSize = 2048
  let destBuffer = UnsafeMutablePointer<UInt8>.allocate(capacity: destBufferSize)
  
  let res = compression_decode_buffer(destBuffer, destBufferSize, source, sourceSize, nil, algorithm);
  
  if res > 0
  {
    result = Data(bytes: destBuffer, count: res)
  }
